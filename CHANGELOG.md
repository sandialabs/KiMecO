...
- The theory score now measures the deviation of a multiplicative parameter in log space, as `ln(x/x0)/ln(f)`, via the new `scoring.get_parameter_deviation`. Additive and percent parameters keep their linear deviation, matching their normal priors and linear boundaries. Theory scores involving multiplicative parameters will differ from earlier versions. `get_parameter_uncertainty_scale` is unchanged and still provides the linear scale for the additive and percent branches.
...
- Scoring a multiplicative parameter linearly made shrinking it cheaper than growing it by the same factor, by a ratio of exactly `f**2` (9x at `f = 3`), and capped the total penalty for driving one to zero at `1/(f-1)**2`. For `f >= 2` that ceiling is below the cost of a single `f`-fold increase — at `f = 3`, annihilating a parameter cost 0.25 against 1.0 for tripling it — so the theory term exerted almost no restraint on shrinkage. This affected `sfc` (default `f = 2.0`) and `mrc` (default `f = 1.5`) on the default settings. Deviations are now symmetric in log space, and both perturbation walls score as exactly `max_std` sigma.
