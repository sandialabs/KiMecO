...
- New `automech` optional-dependency extra (`pip install -e .[automech]` from source, or `pip install kimeco[automech]`) declaring the full automech dependency set (`autoio`, `autochem`, `mako`, `more-itertools`, `networkx`, `pint`, `pyparsing`, `pyyaml`, `qcelemental`, `rdkit`, `scipy`, `xarray`, plus `ipython` and `py3dmol`, which `automol` imports unconditionally) so that `use_automech=true` works once the extra is installed. `autoio`/`autochem` are not published on PyPI, so the documented procedure pre-installs them from the pinned GitHub tags (`autoio@0.2026.0`, `autochem@0.2026.3`) before `pip install -e .[automech]`; `conda install autoio autochem -c auto-mech` remains available as a conda alternative. The `use_automech` missing-dependency warning now points to `pip install kimeco[automech]`.
- New CI tests `tests/unit/test_installation_metadata_ci.py` (packaging metadata, Python floor and documented install-procedure consistency) and `tests/unit/test_installation_imports_ci.py` (import without automech, entry points, absence of the removed agentic package, and an opt-in fresh-venv editable install enabled with `KIMECO_RUN_INSTALL_TEST=1`).
- New CI job `test-automech-extra` (Python 3.11/3.12) that pre-installs `autoio`/`autochem` from the pinned GitHub tags, installs `.[automech,test]`, checks `import mess_io, phydat, kimeco` and runs `tests/unit/` with `KIMECO_REQUIRE_EXTRAS=automech,test`.
- New CI job `test-mess-binary` (Python 3.11/3.12) that builds the `kimeco` environment with micromamba from `conda-forge` + `auto-mech` (`python pip mess-static`, pinned `autoio`/`autochem`, `pip install -e .[automech,test]`), asserts `$(which mess)` is `$CONDA_PREFIX/bin/mess`, and runs `tests/unit/` with `KIMECO_REQUIRE_EXTRAS=automech,test` and `KIMECO_REQUIRE_MESS=1`. It installs MESS but never runs a real MESS input.
- New test module `tests/unit/test_installation_mess_ci.py` checking that `which mess` resolves to exactly one path inside the active interpreter's environment (`sys.prefix/bin/mess`), agrees with `shutil.which`, is a real executable and prints `usage: mess input_file` when run bare, and (via AST) that KiMecO's driver template and SLURM templates invoke the bare `mess` from `PATH`. The new `KIMECO_REQUIRE_MESS` environment variable (`1`/`true`/`yes` → a missing MESS fails; unset/`0`/`false` → skipped; any other value is an error) controls the policy; a `mess` found outside the active environment always fails. `tests/unit/test_installation_metadata_ci.py` now also checks the 6-line recipe (MESS at line 3), the MESS docs section, MANUAL/wiki §5 equality, the lead sentence and the new CI job.
- New test module `tests/unit/test_installation_extras_ci.py` checking that every declared base/`test`/`automech` dependency is importable (including the `IPython`/`py3Dmol` regression) and the automech gate symbols; the new `KIMECO_REQUIRE_EXTRAS` environment variable (comma-separated, case-insensitive; unknown extras are an error) turns the "extra not installed" skips into failures. `tests/unit/test_installation_metadata_ci.py` now also verifies that README, MANUAL and wiki share the same fresh-environment recipe, that the `autoio`/`autochem` git pins match those used in CI, and the hook/CI configuration.
- Install docs (README, MANUAL, wiki `Installation-from-Source`) gained a complete fresh-environment recipe for the `automech` extra (`conda create -n kimeco ... pip`, `conda activate`, `conda install -c auto-mech mess-static -y`, GitHub pre-install of `autoio`/`autochem`, `pip install -e .[automech]`, import check, `which mess`); `pip` is now listed explicitly in the `conda create` lines since conda-forge environments do not include it by default. The recipe previously omitted MESS, so an environment built from it could not run jobs (`FileNotFoundError: 'mess'`); it now yields a complete, job-capable environment.
- The MESS input reader (`kimeco/readers/mess_input.py`) now parses the header keywords `Masses[amu]`, `CalculationMethod`, `ModelEnergyLimit[...]` (stored in kcal/mol; `[1/cm]` and `[kJ/mol]` are converted, an unknown unit logs a warning and keeps the raw value), `ExcessEnergyOverTemperature` and `ChemicalEigenvalueMax` into the new read-only SOP attributes `masses`, `calculation_method`, `model_ene_limit`, `excess_ene_temp` and `chem_eig_max`. With multi-file inputs the first file wins; a mismatch between files (for `Masses[amu]`, only the bath mass at index 0 is compared, the species mass may differ per PES) logs a warning and triggers the existing stop flag, as for `Factor`/`Power`. The template output of the normal pipeline is byte-identical (lines are still copied verbatim).
- The MESS input reader now parses the `RigidRotor` `SymmetryFactor` of wells and bimolecular fragments (`Well.sym_factor`, default 1.0) and of saddle-point barriers (`Barrier.symFact`, default 1.0), and captures the geometry embedded in a `Rotor Hindered` block (`HinRotor.geo`, `None` when absent). `SOP.set_hrotor`, `Well.add_hrotor` and `HinRotor` gained a trailing `geo=None` keyword argument, and the geometry participates in the rotor consistency guard. None of the new attributes enter the parameter database or the perturbation space.
- New CI tests `tests/unit/test_mess_input_header_keys_ci.py`, `tests/unit/test_mess_input_symfact_rotor_geo_ci.py`, `tests/unit/test_automech_payload_symfact_masses_ci.py`, `tests/unit/test_automech_driver_geo_globkey_ci.py` and `tests/unit/test_automech_normal_parity_e2e_ci.py` covering the new reader attributes and the parity between the automech-driven and the normal MESS inputs.

### Changed
- Install docs (README, MANUAL, wiki `Installation-from-Source`) now recommend installing MESS with the `auto-mech` conda package (`conda install -c auto-mech mess-static -y`, environment activated) instead of manually copying static binaries into `$CONDA_PREFIX/bin`, which is kept as a fallback; the `which mess` check now states that it must print a path inside the active environment.
- The automech MESS driver no longer converts pressures at all: instead of rewriting the bar grid into atm, `kimeco/writers/automech_kin.py` now emits the grid unchanged and declares `PressureList[bar]`, with the unit label sourced from `SOP.pres_unit` — the same single source of truth used by the normal MESS pipeline (`kimeco/readers/mess_input.py`) — so the pressures written to the MESS input always match `settings['rc_pres']`. The bar-to-atm conversion, the `_BAR2ATM` factor and the `cantera.with_units` / `ureg` / `Q_` imports are all removed from the writer, and the emitted driver raises a `RuntimeError` if a future `mess_io` stops emitting the hard-coded `PressureList[atm]` literal it patches (rather than silently writing the wrong unit). This fixes the `ValueError: 0.00987 is not in list` raised by `kimeco/readers/mess_output.py` when reading an automech run, where `0.01 bar` had been written as `0.00987 atm` and no longer matched any requested pressure.
- Minimum supported Python version raised from 3.10 to 3.11 (`pyproject.toml`, `setup.py`, `meta.yaml`, `conda_build_config.yaml`, CI matrices now 3.11/3.12, install docs). The floor is the same with or without the `automech` extra, since `autoio`/`autochem` require Python >= 3.11.
- `hooks/run_tests.sh` honours `KIMECO_HOOK_ENV` and falls back to the active interpreter when the hook environment is older than Python 3.11.
- `hooks/run_tests.sh` default candidate environments are now `kmo`, `kimeco`, `automech` (previously `game`, `automech`), matching the environment name used in the install docs.
...
- The automech-driven MESS input (`use_automech=true`, `kimeco/writers/automech_kin.py`) now carries the same chemical information as the normal template pipeline instead of hard-coded placeholders: the `RigidRotor` `SymmetryFactor` of wells and bimolecular fragments is taken from the input (e.g. O2 2, C2H4 4, c-CH2CH2O 2) instead of 1.0; the saddle-point TS `SymmetryFactor` is now `Barrier.symFact` (e.g. 0.5) instead of 1.0; hindered rotors declared on a barrierless `Core Rotd` TS are no longer dropped and are emitted with their own `Geometry[angstrom]` block (mess_io `rotor_hindered(geo=)`); and the Lennard-Jones `Masses[amu]` are taken from the input's `Masses[amu]` line (bath first, species second) instead of always N2 (28.0134) plus the heaviest well mass, the old heuristic remaining only as a fallback with a logged warning.
- The automech-driven MESS input now forwards the global keywords `CalculationMethod`, `ModelEnergyLimit`, `ExcessEnergyOverTemperature` and `ChemicalEigenvalueMax` from the parsed input when present, instead of leaving them to mess_io's defaults (well-reduction / 800 / absent / absent); mess_io defaults still apply when a keyword is absent, and `WellReductionThreshold` is no longer emitted when the method is `direct`.
